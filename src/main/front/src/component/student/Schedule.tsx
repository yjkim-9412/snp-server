...
const Schedule:React.FC<StudentId> = ({studentId}) => {
    ...
    const onChangeCheckBox =  async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!checked) {onRemove(name)}
       await handleIsChecked(name, checked);
    ...
    const onChangeSelect = (selectedDay: string, value: string ) => {
        if ({...isChecked, selectedDay}){handleSelectChange(selectedDay, value);}
    }
    const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        setIsLoading(true);
        e.preventDefault();
        axios.post('/api/schedule/' + studentId, daySchedule).then((res)=> {
            ...
            >
                <Box component="form" onSubmit={onSubmit} noValidate sx={{mt: 3}}>
                    <Grid container flex='auto'>
                        <Grid item xs={6}>
                            <FormGroup>
                                <FormControlLabel control={<Checkbox name='1'checked={isChecked["1"]}  onChange={onChangeCheckBox}/>} label="월요일"/>
                                <FormControlLabel control={<Checkbox name='2'checked={isChecked["2"]}  onChange={onChangeCheckBox}/>} label="화요일"/>
                                <FormControlLabel control={<Checkbox name='3'checked={isChecked["3"]}  onChange={onChangeCheckBox}/>} label="수요일"/>
                                <FormControlLabel control={<Checkbox name='4'checked={isChecked["4"]}  onChange={onChangeCheckBox}/>} label="목요일"/>
                                <FormControlLabel control={<Checkbox name='5'checked={isChecked["5"]}  onChange={onChangeCheckBox}/>} label="금요일"/>
                                <FormControlLabel control={<Checkbox name='6'checked={isChecked["6"]}  onChange={onChangeCheckBox}/>} label="토요일"/>
                            </FormGroup>
                        </Grid>
                        <Grid item xs={2} sm={6}>
                            <TimeSelector selectedDay={dayArray[1]} onChangeSelect={onChangeSelect}
                                          time={daySchedule["1"]}
                                          checkStatus={isChecked["1"]}/>
                            <TimeSelector selectedDay={dayArray[2]} onChangeSelect={onChangeSelect}
                                          time={daySchedule['2']}
                                          checkStatus={isChecked["2"]}/>
                            <TimeSelector selectedDay={dayArray[3]} onChangeSelect={onChangeSelect}
                                          time={daySchedule['3']}
                                          checkStatus={isChecked["3"]}/>
                            <TimeSelector selectedDay={dayArray[4]} onChangeSelect={onChangeSelect}
                                          time={daySchedule['4']}
                                          checkStatus={isChecked["4"]}/>
                            <TimeSelector selectedDay={dayArray[5]} onChangeSelect={onChangeSelect}
                                          time={daySchedule['5']}
                                          checkStatus={isChecked["5"]}/>
                            <TimeSelector selectedDay={dayArray[6]} onChangeSelect={onChangeSelect}
                                          time={daySchedule['6']}
